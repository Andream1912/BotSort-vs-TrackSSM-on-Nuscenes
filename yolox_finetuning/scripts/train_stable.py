...
def main():
    ...
    logger.info(f"\n{'='*50}")
    logger.info(f"Starting ULTRA STABLE training: {exp.max_epoch} epochs")
    logger.info(f"Config: V3 - Maximum stability optimization")
    logger.info(f"Batch size: {exp.batch_size}")
    logger.info(f"LR: {exp.basic_lr_per_img * exp.batch_size:.6f} (40% ridotto)")
    logger.info(f"Warmup: {exp.warmup_epochs} epochs (extended)")
    logger.info(f"No-aug: {exp.no_aug_epochs} epochs (fine-tune phase)")
    logger.info(f"Augmentation: mosaic={exp.mosaic_prob}, mixup={exp.mixup_prob} (minimal)")
    logger.info(f"Scheduler: {exp.scheduler} (cosine annealing)")
